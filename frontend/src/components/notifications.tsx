...
export default function Notifications() {
  ...
  return (
    <Dialog onOpenChange={() => cleanNotificationsCounter()}>
      <DialogTrigger>
        <Button variant={'ghost'} className="relative p-2 rounded-full">
          ...
          {newNotificationCount > 0 && (
            <motion.div
              ...
              className="absolute -top-1 -right-1 bg-blue-600 text-white rounded-full h-5 w-5 flex items-center justify-center text-xs"
            >
              {newNotificationCount}
            </motion.div>
          )}

...
export default function TaskSection({ projectId }: { projectId: number }) {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-3">Tarefas</h2>
        ...
        {tasks?.map(task => (
          <div key={task.id} className="p-3 border rounded-md bg-white">
            ...
            <div className="flex justify-between items-center">
              <p className="mt-1">{task.descricao}</p>
              <Badge
                style={{
                  backgroundColor: projectStatus[task.status].color,
                  color: projectStatus[task.status].textColor,
                }}
                className="text-justify justify-self-end"
              >
                {projectStatus[task.status].text}
              </Badge>
            </div>
          </div>
        ))}
      ...
      <div className="mb-5">
        <AlertDialog open={open} onOpenChange={setOpen}>
          <AlertDialogTrigger asChild>
            <Button className="gap-2 text-base px-4 py-6 rounded-full bg-white">

...
async function getProjectUsers(req: customRequest, res: Response) {
  ...
  if(req.query.page && req.query.limit){
    const users = await projectServices.getProjectUsers(Number(req.params.id), Number(req.query.page), Number(req.query.limit));
    res.status(200).json(users);
  }else{
    const users = await projectServices.getProjectUsers(Number(req.params.id));
    res.status(200).json(users);
  }
}
...
async function postProjectUsers(req: customRequest, res: Response) {
  await userServices.getAuthUser(req.token, req.id)
  const user = await projectServices.postProjectUsers(Number(req.params.id), Number(req.body.usuario_id));
...
async function deleteProjectUsers(req: customRequest, res: Response) {
  ...
  await projectServices.deleteProjectUsers(Number(req.params.id), Number(req.params.usuario_id));
  res.status(204).send();
}



export const projectControllers = { getProjects, createProject, updateProject,deleteProject,getProjectUsers, postProjectUsers, deleteProjectUsers };
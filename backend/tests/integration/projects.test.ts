...
describe("Post projects", () => {
  ...
  it("Should respond with 401 when token is invalid", async () => {
    const response = await server.post("/api/projetos").set('Authorization', 'Bearer invalidtoken');
    expect(response.status).toBe(httpStatus.UNAUTHORIZED);
    expect(response.text).toBe("Token inválido, faça login novamente!");
  })
  ...
  it("Should respond with Unauthorized when jwt is valid but not from the user", async () => {
    const token = jwt.sign({ id: faker.seed(), token: faker.lorem.sentence()}, process.env.JWT_KEY as string, { expiresIn: '1h' });
    const response = await server.post("/api/projetos").set('Authorization', `Bearer ${token}`).send({
...
describe("Update projects", () => {
  ...
  it("Should respond with 401 when token is invalid", async () => {
    const response = await server.put(`/api/projetos/${faker.number.int()}`).set('Authorization', 'Bearer invalidtoken');
    expect(response.status).toBe(httpStatus.UNAUTHORIZED);
    expect(response.text).toBe("Token inválido, faça login novamente!");
  })
  ...
  it("Should respond with 422 when body is invalid", async () => {
    const user = await createFakeUser({ email: faker.internet.email(), senha: '123456' });
    const session = await createFakeSession(user.id);
    const response = await server.put(`/api/projetos/${faker.number.int()}`).set('Authorization', `Bearer ${session}`).send({
      invalidBody: faker.lorem.sentence(),
    });
    expect(response.status).toBe(httpStatus.UNPROCESSABLE_ENTITY);
  })
  ...
  it("Should respond with Unauthorized when jwt is valid but not from the user", async () => {
    const token = jwt.sign({ id: faker.seed(), token: faker.lorem.sentence()}, process.env.JWT_KEY as string, { expiresIn: '1h' });
    const response = await server.put(`/api/projetos/${faker.number.int()}`).set('Authorization', `Bearer ${token}`).send({
      nome: faker.lorem.sentence(),
      descricao: faker.lorem.sentence(),
      status: "Pendente",
      data_inicio: faker.date.recent(),
  ...
  it("Should respond with 400 when initialDate is after end date", async () => {
    ...
    const session = await createFakeSession(user.id);
    const project = await createFakeProject({});
    const response = await server.put(`/api/projetos/${project.id}`).set('Authorization', `Bearer ${session}`).send({
      nome: faker.lorem.sentence(),
      descricao: faker.lorem.sentence(),
      status: "Pendente",
